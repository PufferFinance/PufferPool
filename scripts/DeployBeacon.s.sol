...
contract DeployBeacon is Script {
    function run() external returns (EigenPodProxy, UpgradeableBeacon) {
        ...
        EigenPodProxy eigenPodProxyImplementation = new EigenPodProxy(payable(address(0)), payable(address(0)), payable(address(0)), payable(address(0)), address(0), address(0), 0, 0, 0);
