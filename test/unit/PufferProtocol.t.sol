...
contract PufferProtocolTest is TestHelper {
    ...
    function test_setup() public {
        assertTrue(
            address(pufferProtocol.PUFFER_VAULT()) != address(0),
            "puffer vault address"
        );
        address module = pufferProtocol.getModuleAddress(NO_RESTAKING);
        assertEq(
            PufferModule(payable(module)).NAME(),
            NO_RESTAKING,
            "bad name"
        );
    }
    ...
    function test_empty_queue() public {
        (bytes32 moduleName, uint256 idx) = pufferProtocol
    ...
    function test_skip_provisioning() public {
        ...
        pufferProtocol.skipProvisioning(
            NO_RESTAKING,
            _getGuardianSignaturesForSkipping()
        );

        // This contract should receive pufETH because of the skipProvisioning
        assertTrue(
            pufferVault.balanceOf(address(this)) != 0,
            "non zero pufETH"
        );

        Validator memory aliceValidator = pufferProtocol.getValidatorInfo(
            NO_RESTAKING,
            0
        );
        assertTrue(
            aliceValidator.status == Status.SKIPPED,
        ...
        bytes[] memory signatures = _getGuardianSignatures(
            _getPubKey(bytes32("bob"))
        );

        vm.expectEmit(true, true, true, true);
        emit SuccessfullyProvisioned(
            _getPubKey(bytes32("bob")),
            1,
            NO_RESTAKING
        );
    ...
    function test_register_to_invalid_module() public {
        ...
        ValidatorKeyData memory validatorKeyData = _getMockValidatorKeyData(
            pubKey,
            NO_RESTAKING
        );
        vm.expectRevert(
            IPufferProtocol.ValidatorLimitForModuleReached.selector
        );
        pufferProtocol.registerValidatorKey{value: smoothingCommitment}(
            validatorKeyData,
            bytes32("imaginary module"),
            30,
            emptyPermit,
            emptyPermit
        );
    ...
    function test_module_DOS() external {
        ...
        (bytes32 module, uint256 idx) = pufferProtocol
            .getNextValidatorToProvision();
        assertEq(module, NO_RESTAKING, "module");
        assertEq(idx, 0, "idx");

        pufferProtocol.provisionNode(
            _getGuardianSignatures(_getPubKey(bytes32("alice"))),
            _validatorSignature(),
            0
        );

        uint256 next = pufferProtocol.getNextValidatorToBeProvisionedIndex(
            NO_RESTAKING
        );
        ...
        pufferProtocol.provisionNode(
            _getGuardianSignatures(_getPubKey(bytes32("ford"))),
            _validatorSignature(),
            0
        );

        // That idx is 6
        next = pufferProtocol.getNextValidatorToBeProvisionedIndex(
            NO_RESTAKING
        );
    ...
    function test_fuzz_register_many_validators(
        uint8 numberOfValidatorsToProvision
    ) external {
    ...
    function test_change_module() public {
        ...
        pufferProtocol.changeModule(
            NO_RESTAKING,
            PufferModule(payable(address(5)))
        );
    }

    function test_change_module_to_custom_module() public {
        pufferProtocol.changeModule(
            bytes32("RANDOM_MODULE"),
            PufferModule(payable(address(5)))
        );
        address moduleAfterChange = pufferProtocol.getModuleAddress(
            "RANDOM_MODULE"
        );
    ...
    function test_stop_registration() public {
        ...
        assertEq(
            pufferProtocol.getNextValidatorToBeProvisionedIndex(NO_RESTAKING),
            0,
            "zero index is next in line"
        );
        ...
        pufferProtocol.provisionNode(
            _getGuardianSignatures(_getPubKey(bytes32("bob"))),
            _validatorSignature(),
            0
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IPufferProtocol.InvalidValidatorState.selector,
                Status.DEQUEUED
            )
        );
        pufferProtocol.cancelRegistration(NO_RESTAKING, 0);
    ...
    function test_register_multiple_validator_keys_and_dequeue(
        bytes32 alicePubKeyPart,
        bytes32 bobPubKeyPart
    ) public {
        ...
        Validator memory validator = pufferProtocol.getValidatorInfo(
            NO_RESTAKING,
            0
        );
        assertTrue(validator.node == address(this), "node operator");
        assertTrue(
            keccak256(validator.pubKey) == keccak256(zeroPubKey),
            "bad pubkey"
        );
        ...
        assertEq(
            pufferProtocol.getPendingValidatorIndex(NO_RESTAKING),
            5,
            "next pending validator index"
        );
        ...
        Validator memory bobValidator = pufferProtocol.getValidatorInfo(
            NO_RESTAKING,
            1
        );

        assertTrue(
            bobValidator.status == Status.ACTIVE,
        ...
        // Get validators
        Validator[] memory registeredValidators = pufferProtocol.getValidators(
            NO_RESTAKING
        );
        assertEq(registeredValidators.length, 5, "5 registered validators");
        assertEq(
            registeredValidators[0].node,
            address(this),
            "this contract should be the first one"
        );
        assertEq(
            registeredValidators[1].node,
            bob,
            "bob should be the second one"
        );
        assertEq(
            registeredValidators[2].node,
            alice,
            "alice should be the third one"
        );
        assertEq(
            registeredValidators[3].node,
            address(this),
            "this contract should should be the fourth one"
        );
        assertEq(
            registeredValidators[4].node,
            address(this),
            "this contract should should be the fifth one"
        );
    }
    ...
    function test_provision_node() public {
        ...
        (bytes32 nextModule, uint256 nextId) = pufferProtocol
            .getNextValidatorToProvision();
        ...
        assertEq(
            pufferProtocol.getNextValidatorToBeProvisionedIndex(NO_RESTAKING),
            1,
            "next idx for no restaking module"
        );
    ...
    function test_create_puffer_module() public {
        ...
        IPufferModule module = IPufferModule(
            pufferProtocol.getModuleAddress(name)
        );
    ...
    function test_claim_bond() public {
        ...
        pufferProtocol.provisionNode(
            _getGuardianSignatures(_getPubKey(bytes32("charlie"))),
            _validatorSignature(),
            0
        );

        bytes32[] memory aliceProof = fullWithdrawalsMerkleProof.getProof(
            fullWithdrawalMerkleProofData,
            0
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IPufferProtocol.InvalidMerkleProof.selector)
        );
        ...
        bytes32[] memory bobProof = fullWithdrawalsMerkleProof.getProof(
            fullWithdrawalMerkleProofData,
            1
        );
        ...
        bytes32[] memory charlieProof = fullWithdrawalsMerkleProof.getProof(
            fullWithdrawalMerkleProofData,
            2
        );
        ...
        assertGt(
            pufferVault.maxWithdraw(charlie),
            0.6 ether,
            "Charlie has 0.6 + extra that he earned after"
        );
    }
    ...
    function test_upgrade() public {
        ...
        result = PufferProtocolMockUpgrade(payable(address(pufferProtocol)))
            .returnSomething();
    ...
    function test_register_pufETH_approve_buy_VT() external {
        ...
        pufferProtocol.registerValidatorKey{value: sc}(
            data,
            NO_RESTAKING,
            180,
            permit,
    ...
    function test_register_both_permit() external {
        ...
        uint256 leftOverPufETH = pufferVault.balanceOf(alice) -
            pufferVault.convertToShares(1 ether);
        ...
        assertEq(
            validatorTicket.balanceOf(alice),
            _upscaleTo18Decimals(numberOfDays),
            "VT before for alice"
        );
        ...
        Permit memory pufETHPermit = _signPermit(
            _testTemps("alice", address(pufferProtocol), bond, block.timestamp),
            pufferVault.DOMAIN_SEPARATOR()
        );
        ...
        assertEq(
            pufferVault.balanceOf(alice),
            leftOverPufETH,
            "alice should have some leftover pufETH"
        );
    ...
    function test_register_both_approve() external {
        ...
        // 1 wei diff
        assertApproxEqAbs(
            pufferVault.previewRedeem(pufferVault.balanceOf(alice)),
            1 ether,
            1,
            "1 pufETH before for alice"
        );
    ...
    // Node operator can deposit Bond in pufETH
    function test_register_validator_key_with_permit_reverts_invalid_vt_amount()
        external
    {
        bytes memory pubKey = _getPubKey(bytes32("alice"));
    ...
    function test_validator_griefing_attack() external {
        ...
        _registerValidatorKey(bytes32("alice"), NO_RESTAKING);
        bytes[] memory guardianSignatures = _getGuardianSignatures(
            _getPubKey(bytes32("alice"))
        );
    ...
    function test_validator_limit_per_module() external {
        ...
        vm.expectEmit(true, true, true, true);
        emit IPufferProtocol.ValidatorLimitPerModuleChanged(
            type(uint128).max,
            1
        );
        ...
        pufferProtocol.registerValidatorKey{
            value: (smoothingCommitment + bond)
    ...
    function test_claim_bond_for_single_withdrawal() external {
        ...
        assertApproxEqAbs(
            ...
            1 ether,
            1,
            "~1 pufETH in protocol maxRedeem"
        );
        ...
        pufferProtocol.retrieveBond({
            validatorInfo: validatorInfo,
            merkleProof: fullWithdrawalsMerkleProof.getProof(
                fullWithdrawalMerkleProofData,
                0
            )
        });
        ...
        assertApproxEqAbs(
            ...
            1 ether,
            1,
            "max redeem for alice"
        );
        assertApproxEqAbs(
            pufferVault.previewRedeem(pufferVault.balanceOf(address(alice))),
            1 ether,
            1,
            "alice got back ~1 eth"
        );

        bytes32[] memory proof2 = fullWithdrawalsMerkleProof.getProof(
            fullWithdrawalMerkleProofData,
            1
        );
    ...
    function test_double_deposit_validator_tickets_approval() public {
        ...
        assertEq(
            validatorTicket.balanceOf(address(alice)),
            800 ether,
            "alice got 800"
        );
        assertEq(
            ...
            200 ether,
            "alice got 200 VT in the protocol"
        );
        ...
        assertEq(
            pufferProtocol.getValidatorTicketsBalance(alice),
            1000 ether,
            "alice should have 1000 vt in the protocol"
        );
    }
    ...
    function test_deposit_validator_tickets_permit_for_bob() public {
        ...
        assertEq(
            pufferProtocol.getValidatorTicketsBalance(bob),
            200 ether,
            "bob got the VTS in the protocol"
        );
        assertEq(
            pufferProtocol.getValidatorTicketsBalance(alice),
            0,
            "alice got no VTS in the protocol"
        );
    }
    ...
    function test_double_deposit_validator_tickets_permit_for_bob() public {
        ...
        assertEq(
            validatorTicket.balanceOf(alice),
            800 ether,
            "Alice still has 800 VTs left in wallet"
        );
    ...
    function test_changeMinimumVTAmount() public {
        assertEq(
            pufferProtocol.getMinimumVtAmount(),
            28 ether,
            "initial value"
        );
    ...
    function test_vt_balance_single_validator() public {
        ...
        assertEq(
            balance,
            30 ether,
            "alice should have 30 VTs locked in the protocol"
        );
    ...
    function test_vt_balance_different_provision_time() public {
        ...
        assertEq(
            balance,
            90 ether,
            "alice should have 90 VTs locked in the protocol"
        );
    ...
    function test_withdraw_vt_before_provisioning() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IPufferProtocol.InvalidValidatorTicketAmount.selector,
                30 ether,
                28 ether
            )
        );
        ...
        assertEq(
            pufferProtocol.getValidatorTicketsBalance(alice),
            28 ether,
            "alice got 28 VT in the protocol"
        );
    }
    ...
    function test_stop_validator_provision_another_claim_bond_for_the_first()
        public
    {
        _setupMerkleRoot();
    ...
    function _getGuardianSignatures(
        bytes memory pubKey
    ) internal view returns (bytes[] memory) {
        (bytes32 moduleName, uint256 pendingIdx) = pufferProtocol
            .getNextValidatorToProvision();
        Validator memory validator = pufferProtocol.getValidatorInfo(
            moduleName,
            pendingIdx
        ...
        bytes memory withdrawalCredentials = pufferProtocol
            .getWithdrawalCredentials(validator.module);
    ...
    function _getGuardianSignaturesForSkipping()
        internal
        view
        returns (bytes[] memory)
    {
        ...
        bytes32 digest = LibGuardianMessages._getSkipProvisioningMessage(
            moduleName,
            pendingIdx
        );
    ...
    function _validatorSignature()
        internal
        pure
        returns (bytes memory validatorSignature)
    {
        // mock signature copied from some random deposit transaction
        validatorSignature = hex"8aa088146c8c6ca6d8ad96648f20e791be7c449ce7035a6bd0a136b8c7b7867f730428af8d4a2b69658bfdade185d6110b938d7a59e98d905e922d53432e216dc88c3384157d74200d3f2de51d31737ce19098ff4d4f54f77f0175e23ac98da5";
    }

    function _getMockValidatorKeyData(
        bytes memory pubKey,
        bytes32 moduleName
    ) internal view returns (ValidatorKeyData memory) {
        bytes[] memory newSetOfPubKeys = new bytes[](3);
        ...
        bytes memory withdrawalCredentials = pufferProtocol
            .getWithdrawalCredentials(module);
    ...
    function _singleWithdrawalMerkleRoot() public {
        address NoRestakingModule = pufferProtocol.getModuleAddress(
            NO_RESTAKING
        );
    ...
    function _buildMerkleProof(
        MerkleProofData[] memory validatorExits
        ...
        root = fullWithdrawalsMerkleProof.getRoot(
            fullWithdrawalMerkleProofData
        );
    }
