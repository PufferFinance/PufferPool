 ...
 */
contract SlasherMock is Initializable, OwnableUpgradeable, ISlasher, PausableMock {
    using StructuredLinkedListMock for StructuredLinkedListMock.List;
     ...
     */
    mapping(address => StructuredLinkedListMock.List) internal _operatorToWhitelistedContractsByUpdate;
    ...
    */
        ...
        returns (bool)
    {
         ...
         */
        return (withdrawalStartBlock < update.stalestUpdateBlock && uint32(block.number) > update.latestServeUntilBlock);
    }
    ...
    {
        return StructuredLinkedListMock.getNode(_operatorToWhitelistedContractsByUpdate[operator], _addressToUint(node));
    }
    ...
    function getPreviousWhitelistedContractByUpdate(address operator, uint256 node)
        external
        view
     ...
     */
    function _recordUpdateAndAddToMiddlewareTimes(address operator, uint32 updateBlock, uint32 serveUntilBlock)
        internal
    {
        ...
        // If the middleware is the first in the list, we will update the `stalestUpdateBlock` field in MiddlewareTimes
            ...
            (bool hasNext, uint256 nextNode) =
                _operatorToWhitelistedContractsByUpdate[operator].getNextNode(_addressToUint(msg.sender));

            if (hasNext) {
                // get the next middleware's latest update block
                uint32 nextMiddlewaresLeastRecentUpdateBlock =
                    _whitelistedContractDetails[operator][_uintToAddress(nextNode)].latestUpdateBlock;
    ...
    function _updateMiddlewareList(address operator, uint32 updateBlock, uint256 insertAfter) internal {
        ...
        if (insertAfter != HEAD) {
            ...
            if (!runFallbackRoutine) {
                ...
                (bool hasNext, uint256 nextNode) =
                    _operatorToWhitelistedContractsByUpdate[operator].getNextNode(insertAfter);
                if (hasNext) {
                    /**
            ...
            if (!runFallbackRoutine) {
                ...
                require(
                    _operatorToWhitelistedContractsByUpdate[operator].insertAfter(
                        insertAfter, _addressToUint(msg.sender)
                    ),
             ...
             */
            if (
                _whitelistedContractDetails[operator][_uintToAddress(
                    _operatorToWhitelistedContractsByUpdate[operator].getHead()
