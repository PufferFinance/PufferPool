...
contract EnclaveVerifier is IEnclaveVerifier, RAVE {
    /**
     * @dev RSA Public key for Intel
     *      TODO: LINK to where this can be found online, on official Intel site
     */
    ...
    function verifyGuardianPubKey(
        ...
        bytes32 digest = keccak256(abi.encode(blockhash(blockNumber), pubKey));
        ...
        return _verifyEvidence(payload, evidence, mrenclave, mrsigner);
    }
    ...
    function verifyValidatorPubKey(
        ...
        // Enclave is expected to have hashed in this order
        bytes32 digest = keccak256(abi.encode(blockhash(blockNumber), pubKey)); // TODO this needs to be updated
        bytes memory payload = new bytes(64);
